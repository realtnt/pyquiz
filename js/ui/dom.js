...
(function () {
  ...
  function field(host, label, type, value, onChange, opts) {
    ...
    if (type === "textarea") {
      ...
      // Pill-button radio group. Each option is a label-wrapping input.
      const grp = el("div", { class: "form-radio-group", role: "radiogroup", "aria-label": label });
      const name = "rad-" + Math.random().toString(36).slice(2, 8);
      (arrOpts || []).forEach(([v, l]) => {
        const optId = name + "-" + v;
        const wrap = el("label", { class: "form-radio-pill", for: optId });
        const r = document.createElement("input");
        r.type = "radio";
        r.name = name;
        r.value = String(v);
        r.id = optId;
        if (String(v) === String(value)) r.checked = true;
        r.addEventListener("change", () => { if (r.checked) onChange(r.value); });
        wrap.appendChild(r);
        wrap.appendChild(el("span", null, l));
      ...
      // Syntax-highlighted code editor with line numbers. The textarea
      // is overlaid on a <pre> that renders the highlighted code with
      // matching font metrics; both scroll in sync. The line-numbers
      // gutter sits to the left.
      const cfg = optObj || {};
      ...
      function refreshHighlight() {
        ...
        codeEl.innerHTML = highlightPython(src);
        // Render line numbers — one per newline + one for the trailing
        // line (textareas don't have an implicit final line).
        const lineCount = src.split("\n").length;
        let nums = "";
        for (let i = 1; i <= lineCount; i++) nums += i + "\n";
        gutter.textContent = nums;
      }
      input.addEventListener("input", () => { refreshHighlight(); onChange(input.value); });
  ...
  function highlightPython(src) {
    ...
    while (i < N) {
      ...
      // Numbers — simple: digits with optional . and trailing digits
      if (c >= "0" && c <= "9") {
        let j = i;
        while (j < N && /[0-9_.xXbBoOeE+\-]/.test(src[j])) j++;
  ...
  */
  function setupPaneResize(opts) {
    ...
    setupResizeFor("side-panel", "sp_width", "--sp-w", { side: "left",  min: 240, max: 720 }, opts.persist);
  }
