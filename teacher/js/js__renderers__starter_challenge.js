/* === inlined from js/renderers/starter_challenge.js === */
/* =====================================================================
...
(function () {
  ...
  PyQuiz.Renderers.register("starter_challenge", function (activity, host, cb) {
    ...
    /* The outer student renderer already shows activity.instructions
       above the activity body. Showing payload.instructions here would
       duplicate it (the pack helpers populate both fields with the
       same text). Skip to keep the activity surface uncluttered. */
    ...
    /* The I/O panel above the activity body already shows the first
       example call (Sample call row) and its expected output. Render
       the full Example calls table ONLY when there's more than one
       call to show — otherwise it would just duplicate the panel and
       waste vertical space. Activities with a single example get the
       cleaner panel-only treatment. */
    if ((p.example_calls || []).length > 1) {
