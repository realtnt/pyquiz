    ...
    return {
      ...
      activities: [
        ...
        /* ---------- Section 3: Modify ---------- */
        {
          ...
          title: "= vs == — pick the right one", difficulty: 2,
        ...
        },
        {
          ...
          title: "Missing quotes around a string", difficulty: 2,
        ...
        /* ---------- Section 4: Test & visualise ---------- */
        {
          ...
          title: "Test cases for a greet function", difficulty: 2,
          instructions: "The function takes a name (string) and returns a greeting. Fill in the empty cells. Different rows ask for different things — sometimes the test type, sometimes the data, sometimes the expected output.",
          ...
          hints: [
            { type: "concept", text: "Normal data is a typical case. Boundary data sits at the edge of valid input (empty string is a boundary for strings). Erroneous data is the wrong type or out of range." },
            { type: "nudge",   text: "When only the test type is given, your job is to invent a plausible value of that kind. For erroneous, a non-string like a number will do." }
          ],
          feedback: {
          ...
          payload: {
            ...
            function_name: "greet",
            input_columns: [
              { id: "name", label: "name", type: "str", min_length: 0, max_length: 50, boundary_values: [""] }
            ],
            rows: [
              // Row 1: classic — name + output given, classify the type
              { values: { name: "\"Alice\"", output: "Hello, Alice", test_type: "normal" },   prefilled: ["name", "output"] },
              // Row 2: name + type given, predict the output
              { values: { name: "\"\"",      output: "Hello, ",      test_type: "boundary" }, prefilled: ["name", "test_type"] },
              // Row 3: type + output given, write the data
              { values: { name: "42",         output: "TypeError",    test_type: "erroneous" }, prefilled: ["test_type", "output"] }
            ]
          }
        },
        {
    ...
    return {
      ...
      activities: [
        ...
        /* ---------- Modify ---------- */
        {
          ...
          title: "Assignment used as condition", difficulty: 2,
        ...
        },
        {
          ...
          title: "Missing colon", difficulty: 1,
        ...
        /* ---------- Test & visualise ---------- */
        {
          ...
          title: "Test is_positive", difficulty: 3,
          instructions: "The function returns True when n is greater than 0. Fill in the empty cells. Different rows ask for different things — sometimes the test type, sometimes the data, sometimes the expected output.",
          ...
          estimated_time_seconds: 240,
          hints: [
            { type: "concept", text: "Boundary data sits at or very near the decision point (here, 0). Values like 0, 1 and -1 are boundary; values far from 0 are normal; a non-integer is erroneous." },
            { type: "nudge",   text: "When the type is given as `boundary`, pick a value at or next to 0. When given as `normal`, pick one well away from 0." }
          ],
          feedback: {
          ...
          payload: {
            ...
            input_columns: [
              { id: "n", label: "n", type: "int", min: -1000, max: 1000, decision_boundaries: [0] }
            ],
            rows: [
              // Row 1: classify — value and output given
              { values: { n: "0",       output: "False",     test_type: "boundary"  }, prefilled: ["n", "output"] },
              // Row 2: predict output — value and type given
              { values: { n: "100",     output: "True",      test_type: "normal"    }, prefilled: ["n", "test_type"] },
              // Row 3: design data — type and output given
              { values: { n: "1",       output: "True",      test_type: "boundary"  }, prefilled: ["test_type", "output"] },
              // Row 4: design data — erroneous type given, student picks a non-integer
              { values: { n: "\"abc\"", output: "TypeError", test_type: "erroneous" }, prefilled: ["test_type", "output"] }
            ]
          }
        },
        {
    ...
    return {
      ...
      activities: [
        ...
        /* ---------- Modify ---------- */
        {
          ...
          title: "Missing colon on the for line", difficulty: 1,
        ...
        },
        {
          ...
          title: "Off-by-one in range", difficulty: 2,
        ...
        /* ---------- Test & visualise ---------- */
        {
          ...
          instructions: "The function sums the numbers 0 to n inclusive. Fill in the empty cells. Different rows ask for different things — sometimes the test type, sometimes the data, sometimes the expected output.",
          ...
          estimated_time_seconds: 300,
          hints: [
            { type: "concept", text: "The valid range is 0 to 1000. 0 is a boundary (the minimum). A negative input is erroneous because the function expects non-negative." },
            { type: "nudge",   text: "For 'normal' you should pick a value somewhere in the middle of the valid range. For 'erroneous' anything outside the valid range (or the wrong type) works." }
          ],
          feedback: {
          ...
          payload: {
            ...
            input_columns: [
              { id: "n", label: "n", type: "int", min: 0, max: 1000, decision_boundaries: [0] }
            ],
            rows: [
              // Row 1: classify (n + output given)
              { values: { n: "0",   output: "0",     test_type: "boundary"  }, prefilled: ["n", "output"] },
              // Row 2: predict output (n + type given) — sum 0+1+2+3+4+5 = 15
              { values: { n: "5",   output: "15",    test_type: "normal"    }, prefilled: ["n", "test_type"] },
              // Row 3: predict output (n + type given) — sum 0+1+...+10 = 55
              { values: { n: "10",  output: "55",    test_type: "normal"    }, prefilled: ["n", "test_type"] },
              // Row 4: design data (type + output given) — student picks a value outside 0..1000
              { values: { n: "-1",  output: "Error", test_type: "erroneous" }, prefilled: ["test_type", "output"] }
            ]
          }
        },
        {
