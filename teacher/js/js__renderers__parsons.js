/* === inlined from js/renderers/parsons.js === */
/* =====================================================================
...
(function () {
  ...
  PyQuiz.Renderers.register("parsons", function (activity, host, cb) {
    ...
    /* Expand the compact storage form to the runtime shape we need:
       canonical + distractor lines tagged, accepted orderings ready
    ...
    /* `fixed` was dropped from the new schema (no PRIMM activity used
       it and it complicated the new authoring story). Keep the
       variable name so the rest of the file stays close to the old
       diff but the array is always empty. */
    const fixedSolutionIds = [];
    /* Renderer keeps the canonical IDs as the visual "solution order"
       for reset purposes — the marker handles the actual checking. */
    const solutionOrder = expanded.canonicalIds.slice();
    ...
    return {
      ...
      highlight: function (per_part) {
        /* per_part === null clears the program/bin area marks. */
