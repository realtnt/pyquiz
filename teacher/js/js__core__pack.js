...
(function () {
  ...
  /* -------- drag-reorder helpers --------
     The visual order IS the data order: pack.activities is kept in a
     canonical "bucket" order — unsectioned activities first, then each
     section's activities in section order — so an activity's displayed
     number is just its array position + 1, and moving a section moves its
     whole contiguous block. */
  function bucketKey(act, sectionIds) {
    // Returns the index of the activity's bucket: 0 = unsectioned, then
    // 1..N for sections in pack.sections order. Unknown section → unsectioned.
    const sid = act.section_id;
    if (!sid) return 0;
    const idx = sectionIds.indexOf(sid);
    return idx < 0 ? 0 : idx + 1;
  }
  function normaliseActivityOrder(pack) {
    // Stable-sort activities into canonical bucket order. Within a bucket the
    // existing relative order is preserved.
    if (!Array.isArray(pack.activities)) return;
    const sectionIds = (pack.sections || []).map(s => s.id);
    const withPos = pack.activities.map((a, i) => ({ a: a, i: i, b: bucketKey(a, sectionIds) }));
    withPos.sort((x, y) => (x.b !== y.b) ? x.b - y.b : x.i - y.i);
    pack.activities = withPos.map(x => x.a);
  }
  function activitiesInBucket(pack, sectionId) {
    // sectionId null/"" → unsectioned bucket.
    return pack.activities.filter(a => (sectionId ? a.section_id === sectionId : !a.section_id));
  }
  function placeActivity(pack, activityId, sectionId, indexWithinBucket) {
    // Move an activity into a bucket at a given position within that bucket.
    const a = findActivity(pack, activityId);
    if (!a) return false;
    // Set membership first.
    if (sectionId) a.section_id = sectionId; else delete a.section_id;
    normaliseActivityOrder(pack);
    // Now reposition within its bucket. Pull it out, reinsert at the target
    // index among its bucket-mates, then rebuild the flat array.
    const sectionIds = (pack.sections || []).map(s => s.id);
    const buckets = [[]];
    sectionIds.forEach(() => buckets.push([]));
    pack.activities.forEach(act => { buckets[bucketKey(act, sectionIds)].push(act); });
    const myBucket = sectionId ? (sectionIds.indexOf(sectionId) + 1) : 0;
    const arr = buckets[myBucket];
    const from = arr.indexOf(a);
    if (from >= 0) arr.splice(from, 1);
    let to = (indexWithinBucket == null) ? arr.length : Math.max(0, Math.min(indexWithinBucket, arr.length));
    arr.splice(to, 0, a);
    pack.activities = buckets.reduce((acc, b) => acc.concat(b), []);
    touch(pack);
    return true;
  }
  function reorderSection(pack, sectionId, targetIndex) {
    // Move a section to targetIndex within pack.sections, then reflow the
    // activities so each section's block follows it.
    if (!Array.isArray(pack.sections)) return false;
    const from = pack.sections.findIndex(s => s.id === sectionId);
    if (from < 0) return false;
    const [sec] = pack.sections.splice(from, 1);
    let to = Math.max(0, Math.min(targetIndex, pack.sections.length));
    pack.sections.splice(to, 0, sec);
    renumberSections(pack);
    normaliseActivityOrder(pack);
    touch(pack);
    return true;
  ...
  PyQuiz.Pack = {
    ...
    normaliseActivityOrder: normaliseActivityOrder,
